...
class AESService {
  ...
  static Tuple2<Uint8List, Uint8List> deriveKeyAndIV(
    ...
    var encryptionKey = createUint8ListFromString(passphrase);
    ...
    while (!enoughBytesForKey) {
      // int preHashLength = currentHash.length + encryptionKey.length + salt.length;
      if (currentHash.isNotEmpty) {
        preHash = Uint8List.fromList(currentHash + encryptionKey + salt);
      } else {
        preHash = Uint8List.fromList(encryptionKey + salt);
      }
    ...
    var keyBtyes = concatenatedHashes.sublist(
      0,
      32,
    );
    var ivBtyes = concatenatedHashes.sublist(
      32,
      48,
    );
